...
```cyrus
inline fn add(a: int32, b: int32) int32 { ... }
```
...
```cyrus
callconv(fastcall) fn memcpy(dst: void*, src: void*, n: uint32) void;
```
...
```cyrus
noreturn cold fn fatal_error(msg: uint8*) void { ... }
```
...
```cyrus
extern fn printf(format: const uint8*, ...) int32;
...
```cyrus
extern fn printf(fmt: string, ...) int32 as my_printf;
...
```cyrus
type HandlerFn = fn(int32, int32) void;
```
...
```cyrus
pub fn main() {
    const handler = fn(x: int32, y: int32) int32 {
        return x + y;
    };
...
```cyrus
pub fn main() {
    var add = fn(x: int32, y: int32) int32 {
        return x + y;
    };
...
```cyrus
fn sum(const x: int32, const y: int32) int32 {
    x = 10; // ERROR!
